...
//		}
		...
		if(cm.getCmd() == ClientStatMachine.CMD_0x00){
			//心跳
			// nothing to do 
		}else if(cm.getCmd() == ClientStatMachine.CMD_0x10){
			//单向确认通用信息
			this.clear0x10Message();
			return smList;
		}else if(cm.getCmd() == ClientStatMachine.CMD_0x11){
			//单向确认分组信息
			this.clear0x11Message(cm.getData(), Constant.CLIENT_MESSAGE_MIN_LENGTH);
			return smList;
		}else if(cm.getCmd() == ClientStatMachine.CMD_0x20){
			//单向确认自定义信息
	...
	private void genServerMessageList(ClientMessage cm, ArrayList<ServerMessage> smList) throws Exception {
		...
		if(cm.getSocketAddress() == null){
			...
			if(cm.getSocketAddress().equals(lastAddr)){
				//最新地址和上次地址一致，不回应心跳包
				needTickBack = false;
			}else{
				//地址改变，回应心跳包
